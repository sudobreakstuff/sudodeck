...
void do_action(JsonObject a) {
  const char* t = a["type"]|"";
  if (!strcmp(t,"key")) {
    uint16_t c; bool m; if (key_lookup(a["value"]|"",&c,&m)) { hid_tap(0, c, m); }
  } else if (!strcmp(t,"combo")) {
    uint8_t mod = mod_mask(a["mod"]|"");
    uint16_t c; bool m; if (key_lookup(a["key"]|"",&c,&m)) {
      if (m) { hid_tap(0, c, true); delay(25); hid_release(); }
      else { hid_tap(mod, c, false); delay(25); hid_release(); }
    }
    ...
    if (!name[0]) return;
    if (!strcmp(os, "macos")) {
      hid_tap(KEY_MOD_LGUI, KEY_SPACE, false);
    } else {
      hid_press_mods(KEY_MOD_LGUI);
    }
    delay(30); hid_release();
    delay(150);
    hid_print(name);
    delay(50);
    hid_tap(0, KEY_RETURN, false);
  ...
  while (Serial.available()) {
    ...
    if (c == '\n') {
      ...
      if (serial_buf.length()) {
        ...
        if (l == "{\"cmd\":\"switch_to_json\"}") {
          hid_active = false;
          Serial.println("{\"ok\":true,\"mode\":\"json\"}");
          return;
        }
        if (l == "{\"cmd\":\"switch_to_hid\"}") {
          hid_active = true;
          Serial.println("{\"ok\":true,\"mode\":\"hid\"}");
          return;
        }
      ...
      if (serial_buf.length() < 131072) serial_buf += c;
    }
  }
}
